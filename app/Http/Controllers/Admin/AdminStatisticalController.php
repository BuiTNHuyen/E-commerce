...
class AdminStatisticalController extends Controller
{
    public function index()
    {
        ...
        // 	->get();
        $topProductBuyMonth = Order::with('product:id,pro_name,pro_avatar')
            ->whereMonth('created_at', date('m'))
            ->select(
                \DB::raw('sum(od_qty) as quantity'),
                'od_product_id',
                \DB::raw('od_price') // Sử dụng raw expression để giữ nguyên cột od_price
            )
            ->groupBy('od_product_id', 'od_price') // Thêm od_price vào GROUP BY
            ->limit(20)
            ->orderByDesc('quantity')
            ->get();
